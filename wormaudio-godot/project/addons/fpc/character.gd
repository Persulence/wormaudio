...
#region Sound Export Group

@export var footstep_distance: float = 0.6
@export var running_footstep_distance: float = 1
...
var distance_to_step = footstep_distance
...
func update_footsteps(delta: float):
	var dist = running_footstep_distance if (state == "sprinting") else footstep_distance 
	
	if (is_on_floor()):
		distance_to_step -= (velocity.length() * delta)
	else:
		distance_to_step = footstep_distance
	
	if (!was_on_floor && is_on_floor()):
		play_footstep_sound("landing-1")
	
	if (distance_to_step <= 0):
		#play_footstep_sound("footstep-1")
		play_footstep()
		distance_to_step = dist
	pass
...
func play_footstep():
	...
	footstep.set_position(global_position)
		...
		if sprint_mode == 0:
			if Input.is_action_pressed(controls.SPRINT) and state != "crouching":
				if moving:
					...
					if state == "sprinting":
						enter_normal_state()
			elif state == "sprinting" and !moving:
				enter_normal_state()
		elif sprint_mode == 1:
			if moving:
